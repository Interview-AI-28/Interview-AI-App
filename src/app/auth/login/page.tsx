...
// Minimal shape of the Google Identity Services API we use — the full types
// ship only in @types/google.accounts, which isn't worth adding as a dependency.
...
export default function LoginPage() {
  ...
  // Google's own branded button replaces ours once GIS loads — until then (or
  // if it never loads) the classic redirect-flow button stays as the fallback,
  // so sign-in is never blocked on a third-party script.
  ...
  // Fallback path: classic redirect-based OAuth. Google's consent screen shows
  // the Supabase project's shared *.supabase.co domain in this flow, since the
  // redirect target is Supabase's own callback rather than this site.
  ...
  useEffect(() => {
    ...
    if (!clientId) return // not configured — redirect-flow button stays visible
    ...
    script.onload = async () => {
      ...
      // Google verifies the hashed nonce; Supabase verifies the raw one against
      // the ID token's claim — this pair prevents a captured token being replayed.
      ...
      window.google.accounts.id.initialize({
        ...
        callback: async (response) => {
          ...
          try {
            ...
            // First-login side effects (welcome email, deleted-account
            // re-hydration) — this flow never passes through /auth/callback,
            // so it's the only place these fire for GIS sign-ins. Best-effort:
            // a failure here shouldn't block the user from reaching the app.
            fetch('/api/post-signin', { method: 'POST' }).catch(() => {})
    ...
    <div className="min-h-screen flex overflow-hidden bg-slate-50">
      ...
      <div className="flex-1 flex items-center justify-center bg-slate-50 px-6 py-12">
        <div className="w-full max-w-sm">
          ...
          {/* Google-rendered button (GIS) — hidden until the script loads and
              successfully initializes; the fallback button below covers the gap. */}
          <div ref={buttonContainerRef} className={gsiReady ? 'w-full flex justify-center' : 'hidden'} />
